...
raw_images_present = ~isempty(gui_data.image_path) && ...
    ~isempty(dir(fullfile(gui_data.image_path,'*.png')));
...
if raw_images_present
    n_raw_images = length(dir(fullfile(gui_data.image_path,'*.png')));
else
    n_raw_images = 0;
end
...
if processed_images_present
    n_processed_images = length(dir(fullfile(gui_data.save_path,'*.png')));
else
